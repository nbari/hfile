...
# Compare hfile against b3sum
bench: compare-blake3
...
compare-blake3:
    ...
    bench_file=/tmp/hfile-bench-real.bin
    if [[ ! -f "${bench_file}" ]]; then
        dd if=/dev/zero of="${bench_file}" bs=1M count=256 status=none
    fi

    cargo build --release --locked
    python scripts/compare_blake3.py target/release/hfile "${bench_file}"

compare-blake3-native:
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    CARGO_TARGET_DIR=target/native RUSTFLAGS='-C target-cpu=native' cargo build --release --locked
    python scripts/compare_blake3.py target/native/release/hfile "${bench_file}"
...
check-develop:
    ...
    if [[ "$current_branch" != "develop" ]]; then
        ...
        echo "Switch to develop branch first: git switch develop"
        exit 1
    fi
...
_deploy-merge-and-tag:
    ...
    echo "🔗 Monitor release: https://github.com/nbari/hfile/actions"
